...
for i in range(len(image_time) - 1):
    ...
    if (image_time[i + 1] - image_time[i]) > thresh:
        wheel_time_to_add = np.arange(
            image_time[i], image_time[i + 1], median_image_time
        )
...
plt.title(
    "Gaps in the image times have been filled in\nwith wheel times (when available)"
)
plt.legend()
plt.show()

# Interpolation functions
l_interp_func = interp1d(
    x=wheel_time, y=l_vel, kind="linear", bounds_error=False, fill_value=0
)
r_interp_func = interp1d(
    x=wheel_time, y=r_vel, kind="linear", bounds_error=False, fill_value=0
)
...
plt.plot(
    gapless_image_time[has_image],
    l_vel_interp[has_image],
    "xb",
    label="interpolated velocities from times with images",
)
plt.plot(
    gapless_image_time[np.logical_not(has_image)],
    l_vel_interp[np.logical_not(has_image)],
    "xr",
    label="interpolated velocities from times without images",
)
...
plt.plot(
    gapless_image_time[has_image],
    r_vel_interp[has_image],
    "xb",
    label="interpolated velocities from times with images",
)
plt.plot(
    gapless_image_time[np.logical_not(has_image)],
    r_vel_interp[np.logical_not(has_image)],
    "xr",
    label="interpolated velocities from times without images",
)
...
# write  out
np.savetxt(
    out_path + "interpolated_time.txt",
    np.transpose((has_image.astype(int), gapless_image_time)),
    header="HAS_IMAGE (0=false,1=true), TIME",
)
np.savetxt(
    out_path + "interpolated_wheel_velocities.txt",
    np.transpose((l_vel_interp, r_vel_interp)),
