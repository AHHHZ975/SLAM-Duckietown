...
with open(
    "/home/asha/SLAMDuck/luthor-2019-11-23-19-29-02/data_luthor_2019-11-23-19-29-02/wheel_time_data_luthor_2019-11-23-19-29-02-test.txt"
) as f:
    content = f.readlines()
vel_left = []
vel_right = []
...
for i in range(len(vel_left) - 1):
    ...
    x_k, y_k, theta_k = differential_drive(
        x_new[-1], y_new[-1], theta_new[-1], time_diff[i], vel_left[i], vel_right[i]
    )
    x_new.append(x_k)
    y_new.append(y_k)
