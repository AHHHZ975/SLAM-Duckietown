...
velocities = np.loadtxt(
    "/home/asha/SLAMDuck/data_dec_13/velocity_time_luthor_2019-12-14-00-06-07\
...
T = 0.03
range_bearing_dir = (
    "/home/asha/SLAMDuck/data_dec_13/range_bearing_luthor_2019-12-14-00-06-07/"
)
...
X_estimated = [
    x_0_hat
]  # STATE VECTOR = where we think the robot is + the landmarks [x,y,theta, x1, y2, x2, y2, ....,]
...
for i in range(0, time.shape[0]):
    s = np.random.normal(
        0, 0.001, size * 2
    )  # this is for process noise of the landmark
    s = s.reshape((size, 2))
    ...
    Q_prime_k = np.dot(
        w_jacobian_k, np.dot(w_k, w_jacobian_k.T)
    )  # predicted linearized noise
    P_check_k = (
        np.dot(F_K_1, np.dot(P_hat, F_K_1.T)) + Q_prime_k
    ...
    w, h = 2 * nstd * np.sqrt(vals)
    ell = Ellipse(
        xy=(X_estimated[-1][0], X_estimated[-1][1]),
        width=w,
        height=h,
        angle=angle,
        color="red",
    ...
    if int(time[i][0]) == 1:  # there is an observation
        ...
        landmarks = np.loadtxt(
            range_bearing_dir
            + "range_bearing_frame"
            + format(frame_count, "06")
            + ".txt"
        )
        if landmarks.shape == (3,):  # only 1 landmark
            landmarks = landmarks.reshape(
                1, 3
            )  # reshape from shapeless to a shapely figure #o-la-la
        for j in range(landmarks.shape[0]):  # iterate through the landmark list
            ...
            if int(landmarks[j][0]) in landmark_names:  # if the tag is a legit tag
                ...
                landmark_index = landmark_names.index(
                    int(landmarks[j][0])
                )  # landmark's index in our names list
                landmark_index = (
                    landmark_index * 2 + 3
                )  # landmark's index in the state vector

                th_check_k = wraptopi(X_check_k[2, 0])

                if (
                    int(landmarks[j][0]) in previously_observed_landmarks
                    ...
                    beta = (
                        X_check_k[landmark_index]
                        - X_check_k[0, 0]
                        - d * cos(th_check_k)
                    )
                    gamma = (
                        X_check_k[landmark_index + 1]
                        - X_check_k[1, 0]
                        - d * sin(th_check_k)
                    )
                    alpha = np.sqrt(beta**2 + gamma**2)
                    range_landmark = alpha  # estimated range
                    bearing_landmark = wraptopi(
                        np.arctan2(gamma, beta) - th_check_k
                    )  # estimated bearing

                    measurement_range = float(landmarks[j][1])  # actual range
                    measurement_bearing = wraptopi(
                        float(landmarks[j][2])
                    )  # actual bearing

                else:  # reintialize the start location of the landmark (before we had said it was at zero)
                    previously_observed_landmarks.append(
                        (int(landmarks[j][0]))
                    )  # now that we have seen it, put in in the observed list
                    measurement_range = float(landmarks[j][1])  # actual range
                    measurement_bearing = wraptopi(
                        float(landmarks[j][2])
                    )  # actual bearing
                    ...
                    landmark_x_pose = (
                        ...
                        + d * cos(th_check_k)
                        + measurement_range
                        * cos(wraptopi(wraptopi(measurement_bearing) + th_check_k))
                    )
                    landmark_y_pose = (
                        X_check_k[1, 0]
                        + d * sin(th_check_k)
                        + measurement_range
                        * sin(wraptopi(wraptopi(measurement_bearing) + th_check_k))
                    )
                ...
                ax.annotate(
                    "{:1.0f}".format(landmarks[j][0]),
                    (landmark_x_pose, landmark_y_pose),
                )
                ...
                G_k_landmark = np.array(
                    [
                        [
                            -beta / alpha,
                            -gamma / alpha,
                            (beta * d * sin(th_check_k) - gamma * d * cos(th_check_k))
                            / alpha,
                            beta / alpha,
                            gamma / alpha,
                        ],
                        [
                            gamma / alpha**2,
                            -beta / alpha**2,
                            (-d * sin(th_check_k) * gamma - d * cos(th_check_k) * beta)
                            / alpha**2
                            - 1,
                            -gamma / alpha**2,
                            beta / alpha**2,
                ...
                g_k_landmark = np.array(
                    [[range_landmark], [bearing_landmark]]
                )  # predicted range and bear
                measurements_landmark = np.array(
                    [[measurement_range], [measurement_bearing]]
                )
                ...
                R_prime_k_per_landmark = np.dot(
                    n_jacobian_k, np.dot(n_k, n_jacobian_k.T)
                )

                # the kalman gain:
                k_inside_brackets = np.linalg.inv(
                    np.dot(G_k, np.dot(P_check_k, G_k.T)) + R_prime_k_per_landmark
                )
                ...
                innovation = (
                    measurements_landmark - g_k_landmark
                )  # actual measurements - estimated measurements
    ...
    for landmark_index, lm in enumerate(landmark_names):
        if (
            len(landmarks) == 0 or lm not in landmarks[:, 0].astype(int)
        ) and lm in previously_observed_landmarks:
            ...
            ax.annotate(
                "{:1.0f}".format(lm), (landmark_x_pose, landmark_y_pose), color="gray"
            )
    ...
    plot_arrow_length = 0.05 * np.sqrt(
        (plot_xlims[1] - plot_xlims[0]) ** 2 + (plot_ylims[1] - plot_ylims[0])
    )
