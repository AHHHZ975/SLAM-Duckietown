...
UNIQUE_COLOR = ['#e6194B', '#3cb44b', '#ffe119', '#4363d8', '#f58231', '#911eb4', '#42d4f4', '#f032e6', '#bfef45',
...
def replay(dir):
    ...
    for line in lines[1000:]:
        ...
        if event == "left_wheel":
            ...
            # pass
            bounding_boxes = list(
                map(lambda x: (x.center.tolist(), x.corners.tolist(), x.tag_id, x.pose_t), detected_image))
            visualize_bounding_boxes(img, bounding_boxes)
...
def visualize_bounding_boxes(image, bounding_boxes):
    ...
    for bbox in bounding_boxes:
        ...
        # Plot the center
        ax_img.plot(center[0], center[1], 'ro', label='Center' if 'Center' not in ax_path.get_legend_handles_labels()[
            1] else "")  # Red dot for the center
