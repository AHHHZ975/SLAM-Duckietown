...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Process robot name, ROS bag location, and output directory."
    )
...
def replay(dir):
    ...
    mu_prev = np.array([0.0, 0.0, 0.0])  # x, y, theta
    ...
    for line in lines[1000:]:
        ...
        if delta_timestemp > DELTA_TIME:
            ...
            angular_displacement, linear_displacement = displacement(
                R, baseline, delta_lphi, delta_rphi
            )
...
def estimate_pose2(
    ...
    if angular_displacement == 0:
        ...
        # Move the robot
        mu[0] = mu[0] + inter_vw * (
            np.sin(mu[2] + angular_displacement) - np.sin(mu[2])
        )
        mu[1] = mu[1] + inter_vw * (
            np.cos(mu[2]) - np.cos(mu[2] + angular_displacement)
        )
        ...
        G = np.array(
            [
                [
                    1,
                    0,
                    -inter_vw * np.cos(mu[2])
                    + inter_vw * np.cos(mu[2] + angular_displacement),
                ],
                [
                    0,
                    1,
                    -inter_vw * np.sin(mu[2])
                    + inter_vw * np.sin(mu[2] + angular_displacement),
                ],
                [0, 0, 1],
            ]
        )
...
def displacement(
    ...
    linear_displacement = (
        linear_displacement_wheel_left + linear_displacement_wheel_right
    ) / 2

    angular_displacement = (
        linear_displacement_wheel_right - linear_displacement_wheel_left
    ) / (baseline)
...
def visualize_bounding_boxes(image, bounding_boxes):
    ...
    ax_img.imshow(
        image, cmap="gray"
    )  # Display the image in grayscale if single-channel
    ...
    for bbox in bounding_boxes:
        ...
        ax_img.plot(
            ...
            label=(
                "Center"
                if "Center" not in ax_path.get_legend_handles_labels()[1]
                else ""
            ),
        ...
        ax_img.text(
            ...
            str(
                tag_id
            ),  # + ":" + ",".join(map(lambda x: (f'{x[0]:.2}'), pose_t.tolist())),
